#!/bin/sh
# autoloop installer script
# Usage: curl -sSf https://raw.githubusercontent.com/armgabrielyan/autoloop/main/install.sh | sh
#
...
REPO="armgabrielyan/autoloop"
BINARY_NAME="autoloop"
INSTALL_DIR="${AUTOLOOP_INSTALL_DIR:-$HOME/.local/bin}"
...
install_binary() {
    ...
    archive_name="autoloop-${version}-${target}.${archive_ext}"
    ...
    success "autoloop v$version installed successfully!"

    case ":$PATH:" in
        *":$INSTALL_DIR:"*)
            info "Run 'autoloop --help' to get started"
            ;;
        *)
...
main() {
    echo ""
    echo "  ╭──────────────────────────────────────────────╮"
    echo "  │  autoloop installer                          │"
    echo "  │  agent-agnostic optimization loops           │"
    echo "  ╰──────────────────────────────────────────────╯"
    echo ""
